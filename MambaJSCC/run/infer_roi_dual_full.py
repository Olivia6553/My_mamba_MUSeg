...
def infer_roi_dual_full(
    ...
    max_images=20,
    bg_low_size=64,
    route_mode="pred",      # 新增：pred / all_roi / all_bg
    snr_override=None,      # 新增：指定测试 SNR
    save_visual=False,      # 新增：批量实验默认不保存大量图片
    profile_complexity=True,
):
    assert route_mode in ["pred", "all_roi", "all_bg"], \
    ...
    SNR_roi_profile = snr_override if snr_override is not None else roi_config.CHANNEL.SNR[0]
    SNR_bg_profile = snr_override if snr_override is not None else bg_config.CHANNEL.SNR[0]
    ...
    # )
        ...
        roi_head_macs = 0
        roi_branch_macs_dict = {"branch_macs": 0}
        bg_branch_macs_dict = {"branch_macs": 0}
    ...
    for img_path in tqdm(image_paths):
        ...
        probs = torch.sigmoid(logits)[0, 0]
        pred_grid = (probs >= threshold).float().cpu().numpy().astype(np.uint8)

        # route_grid 用于决定图像块走哪条分支
        if route_mode == "pred":
            grid = pred_grid
        elif route_mode == "all_roi":
            grid = np.ones_like(pred_grid, dtype=np.uint8)
        elif route_mode == "all_bg":
            grid = np.zeros_like(pred_grid, dtype=np.uint8)

        # metric_grid 用于区域 PSNR 统计，保持为 ROI 头预测结果
        # 这样 all_roi / all_bg / pred 三组方法的 ROI/BG 区域 PSNR 使用同一评价区域
        metric_grid = pred_grid

        roi_count = int(grid.sum())
        bg_count = int(56 - roi_count)

        pred_roi_count = int(pred_grid.sum())
        pred_bg_count = int(56 - pred_roi_count)
        ...
        SNR_roi = snr_override if snr_override is not None else roi_config.CHANNEL.SNR[0]
        SNR_bg = snr_override if snr_override is not None else bg_config.CHANNEL.SNR[0]
        ...
        full_mask_roi = grid_to_full_mask(metric_grid)
        full_mask_bg = 1 - full_mask_roi
        ...
        rows.append({
            ...
            "snr": SNR_roi,
            "pred_roi_blocks": pred_roi_count,
            "pred_bg_blocks": pred_bg_count,
